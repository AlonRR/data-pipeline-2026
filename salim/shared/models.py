...
class Store(Base):
    ...
    **``provider`` here is the crawler name** ("shufersal"), not the numeric
    ``ChainId`` that ``provider`` means in every other table in this module.
    That is deliberate — the sources, enrichers and matching all key on the
    crawler name — but it means **the join to prices and promotions is
    ``stores.chain_id`` -> ``prices.provider``, not provider to provider**.
    Both columns are ``String``, so joining provider to provider is accepted by
    Postgres and silently returns nothing. ``chain_id`` is read from the Stores
    file's own ``ChainId`` element and is populated for every row (verified:
    shufersal 7290027600007, rami_levi 7290058140886, yohananof 7290803800003,
    hazi_hinam 7290700100008).
